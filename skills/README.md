...
This directory contains agent skills for the gentleduck/gen ecosystem. Each skill teaches AI coding agents how to work with a specific package -- its API, patterns, coding style, and conventions.
...
|---|---|---|
| `duck-gen` | `@gentleduck/gen` | Code generation CLI, NestJS route scanning, message i18n types, duck-gen.json config, typed client |
...
```bash
npx skills add gentleeduck/duck-gen --skill duck-gen
```
...
The agent only reads reference files when it needs them, keeping the initial context small.
...
1. Create a directory under `skills/` matching the package name:
   ...
   ```bash
   mkdir -p skills/duck-query/references
   ```
...
4. Test the skill locally by symlinking into `.claude/skills/`:
   ...
   ln -s ../../skills/duck-gen .claude/skills/duck-gen
   ```
...
- When a new framework processor is added
...
The docs site exposes an MCP server at `apps/duck-gen-docs/app/api/mcp/` with tools for searching docs, reading component APIs, and getting examples. The `.mcp.json` at the repo root configures this for development.
