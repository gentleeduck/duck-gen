...
const CONTENT_DIR_ENV_VAR = 'DUCK_GEN_DOCS_CONTENT_DIR'
...
const CATEGORY_ENUM = z.enum(['course', 'duck-gen', 'duck-query', 'guides', 'installation', 'general', 'all'])
...
export function createMcpServer(): McpServer {
  const server = new McpServer(
    ...
    },
    {
      ...
      instructions: [
        'MCP server for @gentleduck/gen documentation (gen.gentleduck.org).',
        ...
        'Categories: course, duck-gen, duck-query, guides, installation, general.',
      ].join(' '),
    },
  )
  ...
  server.tool(
    ...
    {
      slug: z.string().describe('Doc page slug, e.g. "duck-gen/overview", "installation/next", "guides/quick-start"'),
      mode: z
        .enum(['full', 'summary'])
        .default('full')
        .describe(
  ...
  server.tool(
    ...
    {
      slug: z.string().describe('Doc page slug, e.g. "duck-gen/overview", "installation/next"'),
    },
    async ({ slug }) => {
